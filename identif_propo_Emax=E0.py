...
for file_name in dir_list:
    ...
    prob = {'f': J, 'x': cas.vertcat(*[C50p, gamma])}
    solver_miso = cas.nlpsol('solver', 'ipopt', prob, opts)

    # %% solves the problem
    sol = solver_miso(x0=[Patient_simu.BisPD.c50p, Patient_simu.BisPD.gamma],
                      lbx=[1, 1 + 1e-33],
                      ubx=[10, 10])
    ...
    for i in range(len(Patient_df)-delay):
        xp = Cep[i+delay]
        xr = 0
        temp = H(xp=xp, xr=xr, C50p=w_opt[0], C50r=1, beta=0, gamma=w_opt[1], Emax=Emax, E0=E0)
        BIS[i] = float(temp['bis'])
        BIS_data[i] = Patient_df.loc[i, 'BIS1']

    if 'BIS_simu' not in Patient_df.columns:
        Patient_df.insert(len(Patient_df.columns), 'BIS_simu', BIS)
    else:
        Patient_df['BIS_simu'] = BIS
    Patient_df.to_csv(data_folder_path + file_name)
    ...
    param_table = pd.DataFrame({'Patient_id': Patient_id, 'C50p': w_opt[0], 'gamma': w_opt[1],
                                'E0': E0, 'Emax': Emax,
                                'Chi_test': bool(P_val < p_value), 'J': float(sol['f'])},
                               index=[0])
...
output_dataframe.to_csv("./outputs/datatable_propo_Emax=E0.csv")
